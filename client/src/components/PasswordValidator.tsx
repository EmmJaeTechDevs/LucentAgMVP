...
export function PasswordValidator({ password, username = "", className = "" }: PasswordValidatorProps) {
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      {isExpanded && (
        <div className="mt-2 p-4 bg-gray-800 rounded-lg space-y-3" data-testid="password-rules-content">
          {rules.map((rule) => (
            <div
              key={rule.id}
...
export const validatePasswordStrength = (password: string, username = ""): boolean => {
  const rules = [
...
export const getPasswordErrors = (password: string, username = ""): string[] => {
  const errors: string[] = [];
