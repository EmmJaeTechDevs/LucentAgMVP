  ...
  const getBuyerId = () => {
    ...
    if (sessionData) {
      try {
        ...
        console.error("Error decrypting buyer session:", error);
      }
    }
    // Fallback to localStorage
    const encryptedUserId = localStorage.getItem("buyerUserId");
    if (encryptedUserId) {
      try {
        return SessionCrypto.decrypt(encryptedUserId);
      } catch (error) {
        console.error("Error decrypting buyer userId:", error);
      }
    }
    return null;
  };
